@extends('admin.layouts.vertical')

@section('title', 'Flatpicker')

@section('extra_css')

@section('page_content')

@php $subTitle = 'Flatpicker'; @endphp
@php $title = 'Form'; @endphp
@include('admin.partials.page-title')

<div class="row">
    <div class="col-xl-9">
        <div class="card">
            <div class="card-body">
                <h5 class="card-title mb-1 anchor" id="overview">
                    Overview <a class="btn btn-sm btn-outline-success rounded-2 float-end"
                                href="https://flatpickr.js.org/#introduction" target="_blank">
                    Official Website </a>
                </h5>
                <p class="text-muted mb-3">flatpickr is a lightweight and powerful datetime
                    picker.</p>
                <h5 class="mt-2">Usage</h5>
                <p class="mb-0">Flatpickr's CSS and Javascript files are bundled in the
                    <code>vendors.min.css</code> and <code>vendors.js</code> and globally
                    included in all pages.</p>
            </div><!-- end card-body -->
        </div><!-- end card -->
        <div class="card">
            <div class="card-body">
                <h5 class="card-title mb-3 anchor" id="basic">
                    Basic<a class="anchor-link" href="#basic">#</a>
                </h5>
                <div class="w-50 mb-3">
                    <div class="mb-3">
                        <input class="form-control" id="basic-datepicker" placeholder="Basic datepicker" type="text"/>
                    </div>
                </div>
                <div class="highlight">
                    <div class="highlight-toolbar">
                        <ul class="nav nav-pills">
                            <li class="nav-item">
                                <a aria-expanded="false" class="nav-link active" data-bs-toggle="tab"
                                   href="#basic-html">
                                    <span class="fs-12 text-uppercase">html</span>
                                </a>
                            </li>
                            <li class="nav-item">
                                <a aria-expanded="true" class="nav-link" data-bs-toggle="tab" href="#basic-javascript">
                                    <span class="fs-12 text-uppercase">javascript</span>
                                </a>
                            </li>
                        </ul>
                        <button class="btn btn-sm btn-outline-success btn-copy-clipboard" data-clipboard-action="copy">
                            Copy
                        </button>
                    </div>
                    <div class="tab-content">
                        <div class="tab-pane show active" id="basic-html">
                            <div class="code" data-simplebar="">
<pre class="language-html">
                                                                 <code>
                                                                      &lt;input type="text" id="basic-datepicker" class="form-control" placeholder="Basic datepicker"&gt;
                                                                 </code>
                                                            </pre>
                            </div>
                        </div>
                        <div class="tab-pane" id="basic-javascript">
                            <div class="code" data-simplebar="">
<pre class="language-javascript">
                                                                 <code>
                                                                      document.getElementById('basic-datepicker').flatpickr();
                                                                 </code>
                                                            </pre>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="card">
            <div class="card-body">
                <h5 class="card-title mb-3 anchor" id="datetime">
                    DateTime<a class="anchor-link" href="#datetime">#</a>
                </h5>
                <div class="w-50 mb-3">
                    <div class="mb-3">
                        <input class="form-control" id="datetime-datepicker" placeholder="Date and Time" type="text"/>
                    </div>
                </div>
                <div class="highlight">
                    <div class="highlight-toolbar">
                        <ul class="nav nav-pills">
                            <li class="nav-item">
                                <a aria-expanded="false" class="nav-link active" data-bs-toggle="tab"
                                   href="#datetime-html">
                                    <span class="fs-12 text-uppercase">html</span>
                                </a>
                            </li>
                            <li class="nav-item">
                                <a aria-expanded="true" class="nav-link" data-bs-toggle="tab"
                                   href="#datetime-javascript">
                                    <span class="fs-12 text-uppercase">javascript</span>
                                </a>
                            </li>
                        </ul>
                        <button class="btn btn-sm btn-outline-success btn-copy-clipboard" data-clipboard-action="copy">
                            Copy
                        </button>
                    </div>
                    <div class="tab-content">
                        <div class="tab-pane show active" id="datetime-html">
                            <div class="code" data-simplebar="">
<pre class="language-html">
                                                                 <code>
                                                                      &lt;input type="text" id="datetime-datepicker" class="form-control" placeholder="Date and Time"&gt;
                                                                 </code>
                                                            </pre>
                            </div>
                        </div>
                        <div class="tab-pane" id="datetime-javascript">
                            <div class="code" data-simplebar="">
<pre class="language-javascript">
                                                                 <code>
                                                                      document.getElementById('datetime-datepicker').flatpickr({
                                                                           enableTime: true,
                                                                           dateFormat: "Y-m-d H:i"
                                                                      });
                                                                 </code>
                                                            </pre>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="card">
            <div class="card-body">
                <h5 class="card-title mb-3 anchor" id="human-friendly-dates">
                    Human-friendly Dates<a class="anchor-link" href="#human-friendly-dates">#</a>
                </h5>
                <div class="w-50 mb-3">
                    <input class="form-control" id="humanfd-datepicker" placeholder="October 9, 2018" type="text"/>
                </div>
                <div class="highlight">
                    <div class="highlight-toolbar">
                        <ul class="nav nav-pills">
                            <li class="nav-item">
                                <a aria-expanded="false" class="nav-link active" data-bs-toggle="tab"
                                   href="#human-friendly-html">
                                    <span class="fs-12 text-uppercase">html</span>
                                </a>
                            </li>
                            <li class="nav-item">
                                <a aria-expanded="true" class="nav-link" data-bs-toggle="tab"
                                   href="#human-friendly-javascript">
                                    <span class="fs-12 text-uppercase">javascript</span>
                                </a>
                            </li>
                        </ul>
                        <button class="btn btn-sm btn-outline-success btn-copy-clipboard" data-clipboard-action="copy">
                            Copy
                        </button>
                    </div>
                    <div class="tab-content">
                        <div class="tab-pane show active" id="human-friendly-html">
                            <div class="code" data-simplebar="">
<pre class="language-html">
                                                                 <code>
                                                                      &lt;input type="text" id="humanfd-datepicker" class="form-control" placeholder="October 9, 2018"&gt;
                                                                 </code>
                                                            </pre>
                            </div>
                        </div>
                        <div class="tab-pane" id="human-friendly-javascript">
                            <div class="code" data-simplebar="">
<pre class="language-javascript">
                                                                 <code>
                                                                      document.getElementById('humanfd-datepicker').flatpickr({
                                                                           altInput: true,
                                                                           altFormat: "F j, Y",
                                                                           dateFormat: "Y-m-d",
                                                                      });
                                                                 </code>
                                                            </pre>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="card">
            <div class="card-body">
                <h5 class="card-title mb-3 anchor" id="minmax">
                    MinDate and MaxDate<a class="anchor-link" href="#minmax">#</a>
                </h5>
                <div class="w-50 mb-3">
                    <input class="form-control" id="minmax-datepicker" placeholder="mindate - maxdate" type="text"/>
                </div>
                <div class="highlight">
                    <div class="highlight-toolbar">
                        <ul class="nav nav-pills">
                            <li class="nav-item">
                                <a aria-expanded="false" class="nav-link active" data-bs-toggle="tab"
                                   href="#minmax-html">
                                    <span class="fs-12 text-uppercase">html</span>
                                </a>
                            </li>
                            <li class="nav-item">
                                <a aria-expanded="true" class="nav-link" data-bs-toggle="tab" href="#minmax-javascript">
                                    <span class="fs-12 text-uppercase">javascript</span>
                                </a>
                            </li>
                        </ul>
                        <button class="btn btn-sm btn-outline-success btn-copy-clipboard" data-clipboard-action="copy">
                            Copy
                        </button>
                    </div>
                    <div class="tab-content">
                        <div class="tab-pane show active" id="minmax-html">
                            <div class="code" data-simplebar="">
<pre class="language-html">
                                                                 <code>
                                                                      &lt;input type="text" id="minmax-datepicker" class="form-control" placeholder="mindate - maxdate"&gt;
                                                                 </code>
                                                            </pre>
                            </div>
                        </div>
                        <div class="tab-pane" id="minmax-javascript">
                            <div class="code" data-simplebar="">
<pre class="language-javascript">
                                                                 <code> 
                                                                      document.getElementById('minmax-datepicker').flatpickr({
                                                                           minDate: "2020-01",
                                                                           maxDate: "2020-03"
                                                                      });
                                                                 </code>
                                                            </pre>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="card">
            <div class="card-body">
                <h5 class="card-title mb-3 anchor" id="multiple">
                    Disabling dates<a class="anchor-link" href="#disable">#</a>
                </h5>
                <div class="w-50 mb-3">
                    <input class="form-control" id="disable-datepicker" placeholder="Disabling dates" type="text"/>
                </div>
                <div class="highlight">
                    <div class="highlight-toolbar">
                        <ul class="nav nav-pills">
                            <li class="nav-item">
                                <a aria-expanded="false" class="nav-link active" data-bs-toggle="tab"
                                   href="#disable-html">
                                    <span class="fs-12 text-uppercase">html</span>
                                </a>
                            </li>
                            <li class="nav-item">
                                <a aria-expanded="true" class="nav-link" data-bs-toggle="tab"
                                   href="#disable-javascript">
                                    <span class="fs-12 text-uppercase">javascript</span>
                                </a>
                            </li>
                        </ul>
                        <button class="btn btn-sm btn-outline-success btn-copy-clipboard" data-clipboard-action="copy">
                            Copy
                        </button>
                    </div>
                    <div class="tab-content">
                        <div class="tab-pane show active" id="disable-html">
                            <div class="code" data-simplebar="">
<pre class="language-html">
                                                                 <code>
                                                                      &lt;input type="text" id="disable-datepicker" class="form-control" placeholder="disable datepicker"&gt;
                                                                 </code>
                                                            </pre>
                            </div>
                        </div>
                        <div class="tab-pane" id="disable-javascript">
                            <div class="code" data-simplebar="">
<pre class="language-javascript">
                                                                 <code>
                                                                      document.getElementById('disable-datepicker').flatpickr({
                                                                           onReady: function () {
                                                                               this.jumpToDate("2025-01")
                                                                           },
                                                                           disable: ["2025-01-10", "2025-01-21", "2025-01-30", new Date(2025, 4, 9) ],
                                                                           dateFormat: "Y-m-d",
                                                                      });
                                                                 </code>
                                                            </pre>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="card">
            <div class="card-body">
                <h5 class="card-title mb-3 anchor" id="multiple">
                    Selecting multiple dates<a class="anchor-link" href="#multiple">#</a>
                </h5>
                <div class="w-50 mb-3">
                    <input class="form-control" id="multiple-datepicker" placeholder="Multiple dates" type="text"/>
                </div>
                <div class="highlight">
                    <div class="highlight-toolbar">
                        <ul class="nav nav-pills">
                            <li class="nav-item">
                                <a aria-expanded="false" class="nav-link active" data-bs-toggle="tab"
                                   href="#multiple-html">
                                    <span class="fs-12 text-uppercase">html</span>
                                </a>
                            </li>
                            <li class="nav-item">
                                <a aria-expanded="true" class="nav-link" data-bs-toggle="tab"
                                   href="#multiple-javascript">
                                    <span class="fs-12 text-uppercase">javascript</span>
                                </a>
                            </li>
                        </ul>
                        <button class="btn btn-sm btn-outline-success btn-copy-clipboard" data-clipboard-action="copy">
                            Copy
                        </button>
                    </div>
                    <div class="tab-content">
                        <div class="tab-pane show active" id="multiple-html">
                            <div class="code" data-simplebar="">
<pre class="language-html">
                                                                 <code>
                                                                      &lt;input type="text" id="multiple-datepicker" class="form-control" placeholder="Multiple dates"&gt;
                                                                 </code>
                                                            </pre>
                            </div>
                        </div>
                        <div class="tab-pane" id="multiple-javascript">
                            <div class="code" data-simplebar="">
<pre class="language-javascript">
                                                                 <code>
                                                                      document.getElementById('multiple-datepicker').flatpickr({
                                                                           mode: "multiple",
                                                                           dateFormat: "Y-m-d"
                                                                      });
                                                                 </code>
                                                            </pre>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="card">
            <div class="card-body">
                <h5 class="card-title mb-3 anchor" id="conjunction">
                    Selecting multiple dates - Conjunction<a class="anchor-link" href="#conjunction">#</a>
                </h5>
                <div class="w-50 mb-3">
                    <input class="form-control" id="conjunction-datepicker" placeholder="2018-10-10 :: 2018-10-11"
                           type="text"/>
                </div>
                <div class="highlight">
                    <div class="highlight-toolbar">
                        <ul class="nav nav-pills">
                            <li class="nav-item">
                                <a aria-expanded="false" class="nav-link active" data-bs-toggle="tab"
                                   href="#conjunction-html">
                                    <span class="fs-12 text-uppercase">html</span>
                                </a>
                            </li>
                            <li class="nav-item">
                                <a aria-expanded="true" class="nav-link" data-bs-toggle="tab"
                                   href="#conjunction-javascript">
                                    <span class="fs-12 text-uppercase">javascript</span>
                                </a>
                            </li>
                        </ul>
                        <button class="btn btn-sm btn-outline-success btn-copy-clipboard" data-clipboard-action="copy">
                            Copy
                        </button>
                    </div>
                    <div class="tab-content">
                        <div class="tab-pane show active" id="conjunction-html">
                            <div class="code" data-simplebar="">
<pre class="language-html">
                                                                 <code>
                                                                      &lt;input type="text" id="conjunction-datepicker" class="form-control" placeholder="2018-10-10 :: 2018-10-11"&gt;
                                                                 </code>
                                                            </pre>
                            </div>
                        </div>
                        <div class="tab-pane" id="conjunction-javascript">
                            <div class="code" data-simplebar="">
<pre class="language-javascript">
                                                                 <code>
                                                                      document.getElementById('conjunction-datepicker').flatpickr({
                                                                           mode: "multiple",
                                                                           dateFormat: "Y-m-d",
                                                                           conjunction: " :: "
                                                                      });
                                                                 </code>
                                                            </pre>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="card">
            <div class="card-body">
                <h5 class="card-title mb-3 anchor" id="range-calendar">
                    Range Calendar<a class="anchor-link" href="#range-calendar">#</a>
                </h5>
                <div class="w-50 mb-3">
                    <input class="form-control" id="range-datepicker" placeholder="2018-10-03 to 2018-10-10"
                           type="text"/>
                </div>
                <div class="highlight">
                    <div class="highlight-toolbar">
                        <ul class="nav nav-pills">
                            <li class="nav-item">
                                <a aria-expanded="false" class="nav-link active" data-bs-toggle="tab"
                                   href="#range-calendar-html">
                                    <span class="fs-12 text-uppercase">html</span>
                                </a>
                            </li>
                            <li class="nav-item">
                                <a aria-expanded="true" class="nav-link" data-bs-toggle="tab"
                                   href="#range-calendar-javascript">
                                    <span class="fs-12 text-uppercase">javascript</span>
                                </a>
                            </li>
                        </ul>
                        <button class="btn btn-sm btn-outline-success btn-copy-clipboard" data-clipboard-action="copy">
                            Copy
                        </button>
                    </div>
                    <div class="tab-content">
                        <div class="tab-pane show active" id="range-calendar-html">
                            <div class="code" data-simplebar="">
<pre class="language-html">
                                                                 <code>
                                                                      &lt;input type="text" id="range-datepicker" class="form-control" placeholder="2018-10-03 to 2018-10-10"&gt;
                                                                 </code>
                                                            </pre>
                            </div>
                        </div>
                        <div class="tab-pane" id="range-calendar-javascript">
                            <div class="code" data-simplebar="">
<pre class="language-javascript">
                                                                 <code>
                                                                      document.getElementById('range-datepicker').flatpickr({
                                                                           mode: "range"
                                                                      });
                                                                 </code>
                                                            </pre>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="card">
            <div class="card-body">
                <h5 class="card-title mb-3 anchor" id="inline-calendar">
                    Inline Calendar<a class="anchor-link" href="#inline-calendar">#</a>
                </h5>
                <div class="w-50 mb-3">
                    <input class="form-control" id="inline-datepicker" placeholder="Inline calendar" type="text"/>
                </div>
                <div class="highlight">
                    <div class="highlight-toolbar">
                        <ul class="nav nav-pills">
                            <li class="nav-item">
                                <a aria-expanded="false" class="nav-link active" data-bs-toggle="tab"
                                   href="#inline-calendar-html">
                                    <span class="fs-12 text-uppercase">html</span>
                                </a>
                            </li>
                            <li class="nav-item">
                                <a aria-expanded="true" class="nav-link" data-bs-toggle="tab"
                                   href="#inline-calendar-javascript">
                                    <span class="fs-12 text-uppercase">javascript</span>
                                </a>
                            </li>
                        </ul>
                        <button class="btn btn-sm btn-outline-success btn-copy-clipboard" data-clipboard-action="copy">
                            Copy
                        </button>
                    </div>
                    <div class="tab-content">
                        <div class="tab-pane show active" id="inline-calendar-html">
                            <div class="code" data-simplebar="">
<pre class="language-html">
                                                                 <code>
                                                                      &lt;input type="text" id="inline-datepicker" class="form-control" placeholder="Inline calendar"&gt;
                                                                 </code>
                                                            </pre>
                            </div>
                        </div>
                        <div class="tab-pane" id="inline-calendar-javascript">
                            <div class="code" data-simplebar="">
<pre class="language-javascript">
                                                                 <code>
                                                                      document.getElementById('inline-datepicker').flatpickr({
                                                                           inline: true
                                                                      });
                                                                 </code>
                                                            </pre>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="card">
            <div class="card-body">
                <h5 class="card-title mb-3 anchor" id="timepicker-basic">
                    Basic Timepicker<a class="anchor-link" href="#timepicker-basic">#</a>
                </h5>
                <div class="w-50 mb-3">
                    <input class="form-control" id="basic-timepicker" placeholder="Basic timepicker" type="text"/>
                </div>
                <div class="highlight">
                    <div class="highlight-toolbar">
                        <ul class="nav nav-pills">
                            <li class="nav-item">
                                <a aria-expanded="false" class="nav-link active" data-bs-toggle="tab"
                                   href="#timepicker-basic-html">
                                    <span class="fs-12 text-uppercase">html</span>
                                </a>
                            </li>
                            <li class="nav-item">
                                <a aria-expanded="true" class="nav-link" data-bs-toggle="tab"
                                   href="#timepicker-basic-javascript">
                                    <span class="fs-12 text-uppercase">javascript</span>
                                </a>
                            </li>
                        </ul>
                        <button class="btn btn-sm btn-outline-success btn-copy-clipboard" data-clipboard-action="copy">
                            Copy
                        </button>
                    </div>
                    <div class="tab-content">
                        <div class="tab-pane show active" id="timepicker-basic-html">
                            <div class="code" data-simplebar="">
<pre class="language-html">
                                                                 <code>
                                                                      &lt;input type="text" id="basic-timepicker" class="form-control" placeholder="Basic timepicker"&gt;
                                                                 </code>
                                                            </pre>
                            </div>
                        </div>
                        <div class="tab-pane" id="timepicker-basic-javascript">
                            <div class="code" data-simplebar="">
<pre class="language-javascript">
                                                                 <code>
                                                                      document.getElementById('basic-timepicker').flatpickr({
                                                                           enableTime: true,
                                                                           noCalendar: true,
                                                                           dateFormat: "H:i"
                                                                      });
                                                                 </code>
                                                            </pre>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="card">
            <div class="card-body">
                <h5 class="card-title mb-3 anchor" id="24hours">
                    24-hour Time Picker<a class="anchor-link" href="#24hours">#</a>
                </h5>
                <div class="w-50 mb-3">
                    <input class="form-control" id="24hours-timepicker" placeholder="16:21" type="text"/>
                </div>
                <div class="highlight">
                    <div class="highlight-toolbar">
                        <ul class="nav nav-pills">
                            <li class="nav-item">
                                <a aria-expanded="false" class="nav-link active" data-bs-toggle="tab"
                                   href="#fullhours-html">
                                    <span class="fs-12 text-uppercase">html</span>
                                </a>
                            </li>
                            <li class="nav-item">
                                <a aria-expanded="true" class="nav-link" data-bs-toggle="tab"
                                   href="#fullhours-javascript">
                                    <span class="fs-12 text-uppercase">javascript</span>
                                </a>
                            </li>
                        </ul>
                        <button class="btn btn-sm btn-outline-success btn-copy-clipboard" data-clipboard-action="copy">
                            Copy
                        </button>
                    </div>
                    <div class="tab-content">
                        <div class="tab-pane show active" id="fullhours-html">
                            <div class="code" data-simplebar="">
<pre class="language-html">
                                                                 <code>
                                                                      &lt;input type="text" id="24hours-timepicker" class="form-control" placeholder="16:21"&gt;
                                                                 </code>
                                                            </pre>
                            </div>
                        </div>
                        <div class="tab-pane" id="fullhours-javascript">
                            <div class="code" data-simplebar="">
<pre class="language-javascript">
                                                                 <code>
                                                                      document.getElementById('24hours-timepicker').flatpickr({
                                                                           enableTime: true,
                                                                           noCalendar: true,
                                                                           dateFormat: "H:i",
                                                                           time_24hr: true
                                                                      });
                                                                 </code>
                                                            </pre>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="card">
            <div class="card-body">
                <h5 class="card-title mb-3 anchor" id="timepicker-minmax">
                    Time Picker w/ Limits<a class="anchor-link" href="#timepicker-minmax">#</a>
                </h5>
                <div class="w-50 mb-3">
                    <input class="form-control" id="minmax-timepicker" placeholder="Limits" type="text"/>
                </div>
                <div class="highlight">
                    <div class="highlight-toolbar">
                        <ul class="nav nav-pills">
                            <li class="nav-item">
                                <a aria-expanded="false" class="nav-link active" data-bs-toggle="tab"
                                   href="#timepicker-minmax-html">
                                    <span class="fs-12 text-uppercase">html</span>
                                </a>
                            </li>
                            <li class="nav-item">
                                <a aria-expanded="true" class="nav-link" data-bs-toggle="tab"
                                   href="#timepicker-minmax-javascript">
                                    <span class="fs-12 text-uppercase">javascript</span>
                                </a>
                            </li>
                        </ul>
                        <button class="btn btn-sm btn-outline-success btn-copy-clipboard" data-clipboard-action="copy">
                            Copy
                        </button>
                    </div>
                    <div class="tab-content">
                        <div class="tab-pane show active" id="timepicker-minmax-html">
                            <div class="code" data-simplebar="">
<pre class="language-html">
                                                                 <code>
                                                                      &lt;input type="text" id="minmax-timepicker" class="form-control" placeholder="Limits"&gt;
                                                                 </code>
                                                            </pre>
                            </div>
                        </div>
                        <div class="tab-pane" id="timepicker-minmax-javascript">
                            <div class="code" data-simplebar="">
<pre class="language-javascript">
                                                                 <code>
                                                                      document.getElementById('minmax-timepicker').flatpickr({
                                                                           enableTime: true,
                                                                           noCalendar: true,
                                                                           dateFormat: "H:i",
                                                                           minDate: "16:00",
                                                                           maxDate: "22:30",
                                                                      });
                                                                 </code>
                                                            </pre>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="card">
            <div class="card-body">
                <h5 class="card-title mb-3 anchor" id="preloading-time">
                    Preloading Time<a class="anchor-link" href="#preloading-time">#</a>
                </h5>
                <div class="w-50 mb-3">
                    <input class="form-control" id="preloading-timepicker" placeholder="Pick a time" type="text"/>
                </div>
                <div class="highlight">
                    <div class="highlight-toolbar">
                        <ul class="nav nav-pills">
                            <li class="nav-item">
                                <a aria-expanded="false" class="nav-link active" data-bs-toggle="tab"
                                   href="#preloading-time-html">
                                    <span class="fs-12 text-uppercase">html</span>
                                </a>
                            </li>
                            <li class="nav-item">
                                <a aria-expanded="true" class="nav-link" data-bs-toggle="tab"
                                   href="#preloading-time-javascript">
                                    <span class="fs-12 text-uppercase">javascript</span>
                                </a>
                            </li>
                        </ul>
                        <button class="btn btn-sm btn-outline-success btn-copy-clipboard" data-clipboard-action="copy">
                            Copy
                        </button>
                    </div>
                    <div class="tab-content">
                        <div class="tab-pane show active" id="preloading-time-html">
                            <div class="code" data-simplebar="">
<pre class="language-html">
                                                                 <code>
                                                                      &lt;input type="text" id="preloading-timepicker" class="form-control" placeholder="Pick a time"&gt;
                                                                 </code>
                                                            </pre>
                            </div>
                        </div>
                        <div class="tab-pane" id="preloading-time-javascript">
                            <div class="code" data-simplebar="">
<pre class="language-javascript">
                                                                 <code>
                                                                      document.getElementById('preloading-timepicker').flatpickr({
                                                                           enableTime: true,
                                                                           noCalendar: true,
                                                                           dateFormat: "H:i",
                                                                           defaultDate: "01:45"
                                                                      });
                                                                 </code>
                                                            </pre>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div> <!-- end col -->
    <div class="col-xl-3">
        <div class="card docs-nav">
            <ul class="nav bg-transparent flex-column">
                <li class="nav-item">
                    <a class="nav-link" href="#overview">Overview</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="#basic">Basic</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="#datetime">DateTime</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="#human-friendly-dates">Human-friendly Dates</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="#minmax">MinDate and MaxDate</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="#disable">Disabling dates</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="#multiple">Selecting multiple dates</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="#conjunction">Selecting multiple dates -
                        Conjunction</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="#range-calendar">Range Calendar</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="#inline-calendar">Inline Calendar</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="#timepicker-basic">Basic Timepicker</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="#24hours">24-hour Time Picker</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="#timepicker-minmax">Time Picker w/ Limits</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="#preloading-time">Preloading Time</a>
                </li>
            </ul>
        </div>
    </div>
</div> <!-- end row -->
@endsection

@section('extra_javascript')
<script src="{{ asset('assets/admin') }}/js/components/form-flatepicker.js"></script>
@endsection
