@extends('admin.layouts.vertical')

@section('title', 'Agent Overview')

@section('extra_css')

@section('page_content')

@php $subTitle = 'Agent Overview'; @endphp
@php $title = 'Real Estate'; @endphp
@include('admin.partials.page-title')

<div class="row">
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header d-flex align-items-center justify-content-between bg-light-subtle flex-wrap">
                <h4 class="card-title">Agent</h4>
                <div class="w-25">
                    <form class="app-search d-none d-md-block">
                        <div class="position-relative">
                            <input autocomplete="off" class="form-control" placeholder="Search" type="search" value=""/>
                            <iconify-icon class="search-widget-icon" icon="solar:magnifer-broken"></iconify-icon>
                        </div>
                    </form>
                </div>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-lg-12">
                        <div class="position-relative">
                            <img alt="" class="img-fluid rounded border border-light border-4"
                                 src="{{ asset('assets/admin') }}/images/properties/p-12.jpg"/>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
<div class="row justify-content-center">
    <div class="col-xl-8 col-lg-12">
        <div class="card">
            <div class="card-body">
                <div class="d-flex flex-wrap align-items-center gap-3">
                    <div class="position-relative">
                        <img alt="avatar-2" class="avatar-xl user-img img-thumbnail rounded-circle"
                             src="{{ asset('assets/admin') }}/images/users/avatar-2.jpg"/>
                        <div class="badge bg-success rounded-2 position-absolute bottom-0 start-50 translate-middle-x mb-n1 fs-11">
                            # 1
                        </div>
                    </div>
                    <div class="d-block">
                        <a class="text-dark fw-medium fs-16" href="#!">Michael A. Miner</a>
                        <p class="mb-0">michaelminer@dayrep.com</p>
                    </div>
                    <div class="ms-lg-auto">
                        <a class="btn btn-primary" href="#!">Message</a>
                        <a class="btn btn-outline-secondary" href="#!">Work With Michael</a>
                    </div>
                </div>
                <div class="mt-3">
                    <p class="d-flex align-items-center gap-2">
                        <iconify-icon class="fs-18 text-primary"
                                      icon="solar:map-point-wave-bold-duotone"></iconify-icon>
                        Lincoln Drive
                        Harrisburg, PA 17101 U.S.A
                    </p>
                    <p class="d-flex align-items-center gap-2">
                        <iconify-icon class="fs-18 text-primary"
                                      icon="solar:outgoing-call-rounded-bold-duotone"></iconify-icon>
                        +123 864-357-7713
                    </p>
                    <h4 class="card-title mb-2 mt-3">Social Media :</h4>
                    <ul class="list-inline d-flex gap-1 mb-0 mt-3 align-items-center">
                        <li class="list-inline-item">
                            <a class="btn btn-soft-primary avatar-sm d-flex align-items-center justify-content-center fs-20"
                               href="javascript: void(0);">
                                <i class="ri-facebook-fill"></i>
                            </a>
                        </li>
                        <li class="list-inline-item">
                            <a class="btn btn-soft-danger avatar-sm d-flex align-items-center justify-content-center fs-20"
                               href="javascript: void(0);">
                                <i class="ri-instagram-line"></i>
                            </a>
                        </li>
                        <li class="list-inline-item">
                            <a class="btn btn-soft-info avatar-sm d-flex align-items-center justify-content-center fs-20"
                               href="javascript: void(0);">
                                <i class="ri-twitter-line"></i>
                            </a>
                        </li>
                        <li class="list-inline-item">
                            <a class="btn btn-soft-success avatar-sm d-flex align-items-center justify-content-center fs-20"
                               href="javascript: void(0);">
                                <i class="ri-whatsapp-line"></i>
                            </a>
                        </li>
                        <li class="list-inline-item">
                            <a class="btn btn-soft-warning avatar-sm d-flex align-items-center justify-content-center fs-20"
                               href="javascript: void(0);">
                                <i class="ri-mail-line"></i>
                            </a>
                        </li>
                    </ul>
                </div>
                <div class="mt-4">
                    <h4 class="card-title mb-2">About Michael :</h4>
                    <p class="mb-2">Meet Michael, a dedicated and experienced real estate agent
                        who is committed to making your real estate journey smooth and
                        successful. With a passion for helping clients achieve their dreams,
                        Michael brings a wealth of knowledge and expertise to every
                        transaction.</p>
                    <p class="mb-2">Michael has been a prominent figure in the real estate
                        industry for over a decade. His career began with a focus on
                        residential properties, quickly expanding to include commercial real
                        estate and investment properties. Michael's extensive experience and
                        deep understanding of the market allow him to navigate even the most
                        complex transactions with ease.</p>
                    <p class="mb-2"><span class="fw-medium text-dark">Agency</span><span class="mx-2">:</span>Universo
                        Realtors</p>
                    <p class="mb-2"><span class="fw-medium text-dark">Agent License</span><span class="mx-2">:</span>LC-5758-2048-3944
                    </p>
                    <p class="mb-2"><span class="fw-medium text-dark">Text Number</span><span class="mx-2">:</span>TC-9275-PC-55685
                    </p>
                    <p class="mb-2"><span class="fw-medium text-dark">Services Area</span><span class="mx-2">:</span>Lincoln
                        Drive Harrisburg</p>
                    <div class="my-2">
                        <a class="link-primary fw-medium" href="#!">Viw More <i class="ri-arrow-right-line"></i></a>
                    </div>
                </div>
                <div class="mt-4">
                    <h4 class="card-title mb-3">Property Status :</h4>
                    <div class="row g-2">
                        <div class="col-xl-4 col-lg-6">
                            <div class="card mb-0 shadow-none border">
                                <div class="card-body">
                                    <div class="row justify-content-between align-items-center">
                                        <div class="col-xl-5">
                                            <div class="avatar bg-primary bg-opacity-10 rounded mb-3">
                                                <iconify-icon class="fs-28 text-primary avatar-title"
                                                              icon="solar:home-bold"></iconify-icon>
                                            </div>
                                            <p class="fw-medium fs-15 mb-1">Total Listing
                                            </p>
                                            <p class="mb-0 fw-semibold text-dark fs-20">
                                                243</p>
                                        </div>
                                        <div class="col-lg-6">
                                            <div class="apex-charts" id="property_sale">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-xl-4 col-lg-6">
                            <div class="card mb-0 shadow-none border">
                                <div class="card-body">
                                    <div class="row justify-content-between align-items-center">
                                        <div class="col-xl-5">
                                            <div class="avatar bg-primary bg-opacity-10 rounded mb-3">
                                                <iconify-icon class="fs-28 text-primary avatar-title"
                                                              icon="solar:wallet-money-bold"></iconify-icon>
                                            </div>
                                            <p class="fw-medium fs-15 mb-1">Property Sold
                                            </p>
                                            <p class="mb-0 fw-semibold text-dark fs-20">
                                                136</p>
                                        </div>
                                        <div class="col-xl-6">
                                            <div class="apex-charts" id="property_sale2">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-xl-4 col-lg-6">
                            <div class="card mb-0 shadow-none border">
                                <div class="card-body">
                                    <div class="row justify-content-between align-items-center">
                                        <div class="col-xl-5">
                                            <div class="avatar bg-primary bg-opacity-10 rounded mb-3">
                                                <iconify-icon class="fs-28 text-primary avatar-title"
                                                              icon="solar:hand-money-bold"></iconify-icon>
                                            </div>
                                            <p class="fw-medium fs-15 mb-1">Property Rent
                                            </p>
                                            <p class="mb-0 fw-semibold text-dark fs-20">
                                                105</p>
                                        </div>
                                        <div class="col-lg-6">
                                            <div class="apex-charts" id="property_sale3">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="mt-4">
                    <h4 class="card-title mb-3">Reviews :</h4>
                    <div class="row">
                        <div class="col-lg-6">
                            <div class="card bg-light-subtle mb-0">
                                <div class="card-body">
                                    <div class="d-flex align-items-center gap-2">
                                        <img alt="" class="rounded-circle avatar-md"
                                             src="{{ asset('assets/admin') }}/images/users/avatar-3.jpg"/>
                                        <div>
                                            <h5 class="fw-semibold text-dark mb-1">Esther
                                                Howard</h5>
                                            <p class="m-0">@kasyrhh <span class="ms-1">(U.S.A)</span></p>
                                        </div>
                                    </div>
                                    <p class="my-3">"The team at agent went above and
                                        beyond to
                                        understand my needs and tailor their solutions to
                                        fit my business
                                        requirements Not only did they exceed our
                                        expectations "</p>
                                    <ul class="d-flex text-warning m-0 fs-18 list-unstyled">
                                        <li>
                                            <i class="ri-star-fill"></i>
                                        </li>
                                        <li>
                                            <i class="ri-star-fill"></i>
                                        </li>
                                        <li>
                                            <i class="ri-star-fill"></i>
                                        </li>
                                        <li>
                                            <i class="ri-star-fill"></i>
                                        </li>
                                        <li>
                                            <i class="ri-star-half-line"></i>
                                        </li>
                                    </ul>
                                    <p class="fw-medium text-muted mb-0">3 Days Ago</p>
                                </div>
                            </div>
                        </div>
                        <div class="col-lg-6">
                            <div class="card bg-light-subtle mb-0">
                                <div class="card-body">
                                    <div class="d-flex align-items-center gap-2">
                                        <img alt="" class="rounded-circle avatar-md"
                                             src="{{ asset('assets/admin') }}/images/users/avatar-4.jpg"/>
                                        <div>
                                            <h5 class="fw-semibold text-dark mb-1">Elbart
                                                Bant</h5>
                                            <p class="m-0">@ryejghf <span class="ms-1">(Canada)</span></p>
                                        </div>
                                    </div>
                                    <p class="my-3">"The agent team exceeded expectations
                                        in my needs and customizing their solutions to
                                        perfectly align with my business requirements
                                        game-changer for our business."</p>
                                    <ul class="d-flex text-warning m-0 fs-18 list-unstyled">
                                        <li>
                                            <i class="ri-star-fill"></i>
                                        </li>
                                        <li>
                                            <i class="ri-star-fill"></i>
                                        </li>
                                        <li>
                                            <i class="ri-star-fill"></i>
                                        </li>
                                        <li>
                                            <i class="ri-star-fill"></i>
                                        </li>
                                        <li>
                                            <i class="ri-star-half-line"></i>
                                        </li>
                                    </ul>
                                    <p class="fw-medium text-muted mb-0">15 Days Ago</p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <h4 class="card-title mt-4">Property File :</h4>
                <div class="mt-3 d-flex flex-wrap gap-2">
                    <div class="d-flex p-2 gap-2 bg-light-subtle align-items-center text-start position-relative border rounded">
                        <iconify-icon class="text-danger fs-24" icon="solar:file-check-bold"></iconify-icon>
                        <div>
                            <h4 class="fs-14 mb-1"><a class="text-dark stretched-link" href="#!">Property-File.pdf</a>
                            </h4>
                            <p class="fs-12 mb-0">2.4 MB</p>
                        </div>
                        <i class="ri-download-cloud-line fs-20 text-muted"></i><i class=""></i>
                    </div>
                    <div class="d-flex p-2 gap-2 bg-light-subtle align-items-center text-start position-relative border rounded">
                        <iconify-icon class="text-primary fs-24" icon="solar:user-bold"></iconify-icon>
                        <div>
                            <h4 class="fs-14 mb-1"><a class="text-dark stretched-link" href="#!">Client-List.pdf</a>
                            </h4>
                            <p class="fs-12 mb-0">1.6 MB</p>
                        </div>
                        <i class="ri-download-cloud-line fs-20 text-muted"></i>
                    </div>
                    <div class="d-flex p-2 gap-2 bg-light-subtle align-items-center text-start position-relative border rounded">
                        <iconify-icon class="text-success fs-24" icon="solar:gallery-minimalistic-bold"></iconify-icon>
                        <div>
                            <h4 class="fs-14 mb-1"><a class="text-dark stretched-link" href="#!">Property-Photo.pdf</a>
                            </h4>
                            <p class="fs-12 mb-0">23.2 MB</p>
                        </div>
                        <i class="ri-download-cloud-line fs-20 text-muted"></i>
                    </div>
                    <div class="d-flex p-2 gap-2 bg-light-subtle align-items-center text-start position-relative border rounded">
                        <iconify-icon class="text-warning fs-24" icon="solar:streets-map-point-bold"></iconify-icon>
                        <div>
                            <h4 class="fs-14 mb-1"><a class="text-dark stretched-link" href="#!">Area-sqft.png</a></h4>
                            <p class="fs-12 mb-0">2.3 MB</p>
                        </div>
                        <i class="ri-download-cloud-line fs-20 text-muted"></i>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="col-xl-4 col-lg-12">
        <div class="card bg-light-subtle overflow-hidden z-1">
            <div class="card-body">
                <div class="d-flex align-items-center gap-2 mb-3">
                    <img alt="" class="avatar-md rounded-circle"
                         src="{{ asset('assets/admin') }}/images/users/avatar-2.jpg"/>
                    <div class="d-block">
                        <h5 class="text-dark fw-medium">Michael A. Miner</h5>
                        <p class="mb-0 fw-medium">#1 Medal</p>
                    </div>
                    <div class="ms-auto">
                        <div class="avatar bg-primary-subtle rounded">
                            <iconify-icon class="fs-28 text-primary avatar-title"
                                          icon="solar:cup-star-bold"></iconify-icon>
                        </div>
                    </div>
                </div>
                <div class="p-3 position-relative overflow-hidden z-1 rounded bg-primary-subtle border border-primary-subtle mt-4 text-center">
                    <img alt="" class="mx-auto" height="150" src="{{ asset('assets/admin') }}/images/trophy.png"
                         width="150"/>
                    <span class="position-absolute top-0 end-0 m-1 badge text-light bg-danger px-2 py-1 fs-12">#
                                                  1</span>
                    <div class="position-absolute top-50 start-50 translate-middle z-n1 opacity-50">
                        <svg height="400" version="1.1" viewbox="0 0 800 800" width="400"
                             xmlns="http://www.w3.org/2000/svg" xmlns:svgjs="http://svgjs.dev/svgjs"
                             xmlns:xlink="http://www.w3.org/1999/xlink">
                            <defs>
                                <filter height="400%" id="bbburst-blur-1" width="400%" x="-100%" y="-100%">
                                    <fegaussianblur in="SourceGraphic" stddeviation="1">
                                    </fegaussianblur>
                                </filter>
                                <filter height="400%" id="bbburst-blur-2" width="400%" x="-100%" y="-100%">
                                    <fegaussianblur in="SourceGraphic" stddeviation="2">
                                    </fegaussianblur>
                                </filter>
                                <filter height="400%" id="bbburst-blur-3" width="400%" x="-100%" y="-100%">
                                    <fegaussianblur in="SourceGraphic" stddeviation="4">
                                    </fegaussianblur>
                                </filter>
                                <filter height="400%" id="bbburst-blur-4" width="400%" x="-100%" y="-100%">
                                    <fegaussianblur in="SourceGraphic" stddeviation="12">
                                    </fegaussianblur>
                                </filter>
                                <symbol id="bbburst-shape-1" viewbox="0 0 194 167">
                                    <path d="m97 0 96.129 166.5H.871L97 0Z"></path>
                                </symbol>
                                <symbol id="bbburst-shape-4" viewbox="0 0 149 143">
                                    <path d="M71.647 2.781c.898-2.764 4.808-2.764 5.706 0l15.445 47.534a3 3 0 0 0 2.853 2.073h49.98c2.906 0 4.115 3.719 1.764 5.427L106.96 87.193a2.999 2.999 0 0 0-1.09 3.354l15.445 47.534c.898 2.764-2.266 5.062-4.617 3.354l-40.435-29.378a3 3 0 0 0-3.526 0l-40.435 29.378c-2.351 1.708-5.515-.59-4.617-3.354L43.13 90.547a3 3 0 0 0-1.09-3.354L1.605 57.815c-2.35-1.708-1.142-5.427 1.764-5.427h49.98a3 3 0 0 0 2.853-2.073L71.647 2.781Z">
                                    </path>
                                </symbol>
                                <symbol id="bbburst-shape-6" viewbox="0 0 133 116">
                                    <path d="M59.7487 10.2513c-13.6683-13.66839-35.8291-13.66839-49.4974 0-13.66839 13.6683-13.66839 35.8291 0 49.4974l49.4974-49.4974ZM66.5 66.5 41.7513 91.2487 66.5 115.997l24.7487-24.7483L66.5 66.5Zm56.249-6.7513c13.668-13.6683 13.668-35.8291 0-49.4974-13.669-13.66839-35.8294-13.66839-49.4977 0l49.4977 49.4974Zm-112.4977 0 31.5 31.5 49.4974-49.4974-31.5-31.5-49.4974 49.4974Zm80.9974 31.5 31.5003-31.5-49.4977-49.4974-31.5 31.5 49.4974 49.4974Z">
                                    </path>
                                </symbol>
                                <symbol id="bbburst-shape-8" viewbox="0 0 87 168">
                                    <path d="m12 12 31.2546 18.0749c16.4102 9.4902 20.014 31.6325 7.4603 45.8369L36.3296 92.1884c-12.5684 14.2206-8.9394 36.3916 7.5068 45.8636L75 156"
                                          stroke-linecap="round" stroke-linejoin="round" stroke-width="14"></path>
                                </symbol>
                                <symbol id="bbburst-shape-10" viewbox="0 0 145 145">
                                    <circle cx="72.5" cy="72.5" fill="none" r="61.5" stroke-width="22"></circle>
                                </symbol>
                            </defs>
                            <use fill="#ff5c58" filter="url(#bbburst-blur-4)" opacity="-0.07170930593208524"
                                 transform="matrix(1.6526711594398502,-0.5754807023313131,0.5754807023313131,1.6526711594398502,-208.95529848116416,-587.3516463083347)"
                                 width="42.7645635972241" xlink:href="#bbburst-shape-6"></use>
                            <use fill="#00a19d" filter="url(#bbburst-blur-1)" opacity="0.6238328430200086"
                                 transform="matrix(0.9386727757288379,0.3448092517690307,-0.3448092517690307,0.9386727757288379,532.9877083917494,-148.83342225578508)"
                                 width="24.45268163114131" xlink:href="#bbburst-shape-6"></use>
                            <use fill="#00a19d" filter="url(#bbburst-blur-3)" opacity="0.19126324041346"
                                 transform="matrix(1.2281308116653258,0.23279757180492255,-0.23279757180492255,1.2281308116653258,750.8439830146688,141.92007821728635)"
                                 width="30.454276035972466" xlink:href="#bbburst-shape-6"></use>
                            <use fill="#ffbd9b" filter="url(#bbburst-blur-3)" opacity="0.12187606864879164"
                                 transform="matrix(0.9545507788420095,0.8070519255984172,-0.8070519255984172,0.9545507788420095,870.4769229770454,-353.8497529328647)"
                                 width="27.488636503494444" xlink:href="#bbburst-shape-4"></use>
                            <use fill="#ffa6d5" filter="url(#bbburst-blur-2)" opacity="0.5112244876552516"
                                 transform="matrix(0.915706425996272,0.40184790828015254,-0.40184790828015254,0.915706425996272,346.30260123269477,-65.01300650116087)"
                                 width="32.254817717847224" xlink:href="#bbburst-shape-6"></use>
                            <use fill="none" opacity="0.7980318861011595" stroke="#ffa6d5"
                                 transform="matrix(0.9276234643061629,0.37351667763117724,-0.37351667763117724,0.9276234643061629,449.4818342922476,32.98892380865095)"
                                 width="20.21442586691213" xlink:href="#bbburst-shape-8"></use>
                            <use fill="#ff5c58" filter="url(#bbburst-blur-3)" opacity="0.18702378771218708"
                                 transform="matrix(1.1278902211961048,-0.5388540144883416,0.5388540144883416,1.1278902211961048,534.5821125515372,-26.02576397983802)"
                                 width="27.134020468164294" xlink:href="#bbburst-shape-1"></use>
                            <use fill="#ffa6d5" filter="url(#bbburst-blur-2)" opacity="0.5485864014593227"
                                 transform="matrix(0.7532193948720056,0.6577693692994907,-0.6577693692994907,0.7532193948720056,689.3291782131198,288.12242233703654)"
                                 width="30.97345221107374" xlink:href="#bbburst-shape-1"></use>
                            <use fill="none" filter="url(#bbburst-blur-3)" opacity="0.19337165589242467"
                                 stroke="#2c2891"
                                 transform="matrix(1.113325535279088,-0.5683363902615526,0.5683363902615526,1.113325535279088,133.7553925153216,324.3477668784682)"
                                 width="26.405381228888885" xlink:href="#bbburst-shape-10">
                            </use>
                            <use fill="#ffa6d5" filter="url(#bbburst-blur-1)" opacity="0.6268740089471321"
                                 transform="matrix(0.7910466730728583,-0.6117558017872511,0.6117558017872511,0.7910466730728583,-16.015932347447375,49.99927627938513)"
                                 width="20.80218050506912" xlink:href="#bbburst-shape-6"></use>
                            <use fill="none" filter="url(#bbburst-blur-2)" opacity="0.5053598408190325" stroke="#00a19d"
                                 transform="matrix(0.778377828694753,0.6277961100520151,-0.6277961100520151,0.778377828694753,818.185421062104,211.88647181084565)"
                                 width="29.996757481902492" xlink:href="#bbburst-shape-8">
                            </use>
                            <use fill="#ffa6d5" filter="url(#bbburst-blur-1)" opacity="0.6889166670322748"
                                 transform="matrix(0.999889136446447,-0.014890091214584187,0.014890091214584187,0.999889136446447,502.87136921670486,-75.22948530623756)"
                                 width="18.207236504599862" xlink:href="#bbburst-shape-4"></use>
                            <use fill="none" filter="url(#bbburst-blur-4)" opacity="0.029175559445910992"
                                 stroke="#ff5c58"
                                 transform="matrix(1.7430963796322434,0.15529008761980498,-0.15529008761980498,1.7430963796322434,792.4076091621677,-52.21662505298559)"
                                 width="33.96163868797385" xlink:href="#bbburst-shape-10">
                            </use>
                            <use fill="#ff5c58" filter="url(#bbburst-blur-2)" opacity="0.5000492880886687"
                                 transform="matrix(0.9051613828988044,-0.42506807796954593,0.42506807796954593,0.9051613828988044,199.14980915441092,-180.3241119534908)"
                                 width="28.056793640934977" xlink:href="#bbburst-shape-4"></use>
                            <use fill="none" filter="url(#bbburst-blur-2)" opacity="0.4206325642902956" stroke="#2c2891"
                                 transform="matrix(0.9921317668843107,0.125198071626586,-0.125198071626586,0.9921317668843107,677.0384399266816,-102.06391867096653)"
                                 width="24.716309224385068" xlink:href="#bbburst-shape-10">
                            </use>
                            <use fill="none" filter="url(#bbburst-blur-2)" opacity="0.42845409874613516"
                                 stroke="#2c2891"
                                 transform="matrix(0.9921766508252325,0.12484187421384131,-0.12484187421384131,0.9921766508252325,189.62192376834145,86.08425987065573)"
                                 width="35.541517130951746" xlink:href="#bbburst-shape-10">
                            </use>
                            <use fill="#2c2891" filter="url(#bbburst-blur-3)" opacity="0.3258437211029286"
                                 transform="matrix(0.944841087078961,-0.8183980206277673,0.8183980206277673,0.944841087078961,-248.459221879349,48.320986967694125)"
                                 width="38.01068509323866" xlink:href="#bbburst-shape-1"></use>
                            <use fill="#ffa6d5" filter="url(#bbburst-blur-1)" opacity="0.5902447666423021"
                                 transform="matrix(0.9510651348898608,-0.30899046780917194,0.30899046780917194,0.9510651348898608,190.07567800269965,-145.45146590347088)"
                                 width="29.24502412758138" xlink:href="#bbburst-shape-4"></use>
                            <use fill="#ffa6d5" filter="url(#bbburst-blur-3)" opacity="0.19826035614605375"
                                 transform="matrix(1.1311722104551456,-0.5319299110728968,0.5319299110728968,1.1311722104551456,-83.49863473058173,227.51215700873922)"
                                 width="49.36590703849126" xlink:href="#bbburst-shape-6"></use>
                            <use fill="#ffbd9b" filter="url(#bbburst-blur-1)" opacity="0.5738512230056199"
                                 transform="matrix(0.8425221768277505,-0.5386616577717673,0.5386616577717673,0.8425221768277505,4.749333228200555,162.35483386422334)"
                                 width="27.360087102678115" xlink:href="#bbburst-shape-6"></use>
                            <use fill="none" opacity="0.9153724768144158" stroke="#ffa6d5"
                                 transform="matrix(0.994088205753386,0.10857549991602099,-0.10857549991602099,0.994088205753386,426.89945260671476,-35.27332354094278)"
                                 width="14.09970057460975" xlink:href="#bbburst-shape-10"></use>
                            <use fill="#ffbd9b" filter="url(#bbburst-blur-2)" opacity="0.3604346911205569"
                                 transform="matrix(0.9934409432373326,0.11434636985807187,-0.11434636985807187,0.9934409432373326,623.6206098128381,214.49544322076844)"
                                 width="30.263128319955847" xlink:href="#bbburst-shape-4"></use>
                            <use fill="#00a19d" opacity="0.7872019667652703"
                                 transform="matrix(0.9530509308877397,-0.3028100446385701,0.3028100446385701,0.9530509308877397,364.98588930164493,34.8779712475565)"
                                 width="18.315561816155817" xlink:href="#bbburst-shape-1"></use>
                            <use fill="#2c2891" filter="url(#bbburst-blur-2)" opacity="0.46266843907030153"
                                 transform="matrix(0.8768788161284009,0.48071149541617375,-0.48071149541617375,0.8768788161284009,809.3199274646857,119.18334004633482)"
                                 width="28.010213615578664" xlink:href="#bbburst-shape-1"></use>
                            <use fill="#2c2891" filter="url(#bbburst-blur-2)" opacity="0.42309942111380605"
                                 transform="matrix(0.9996886767191043,0.024950944663602995,-0.024950944663602995,0.9996886767191043,309.351111289664,-245.85664485141194)"
                                 width="33.09795791994925" xlink:href="#bbburst-shape-1"></use>
                            <use fill="#ff5c58" filter="url(#bbburst-blur-2)" opacity="0.3393258208844824"
                                 transform="matrix(0.9699793144683696,0.24318743697706047,-0.24318743697706047,0.9699793144683696,196.7523053739348,-87.48795535330828)"
                                 width="38.64139138894649" xlink:href="#bbburst-shape-4"></use>
                            <use fill="#00a19d" filter="url(#bbburst-blur-2)" opacity="0.37611505023029734"
                                 transform="matrix(0.9657506004722615,0.2594721135063773,-0.2594721135063773,0.9657506004722615,694.7326712900791,-177.7897691715119)"
                                 width="39.21931205625753" xlink:href="#bbburst-shape-1"></use>
                            <use fill="none" filter="url(#bbburst-blur-1)" opacity="0.7241258630588089" stroke="#00a19d"
                                 transform="matrix(0.9998236559228544,0.01877916550482671,-0.01877916550482671,0.9998236559228544,292.033710211246,70.40506156898095)"
                                 width="26.71293849985137" xlink:href="#bbburst-shape-8">
                            </use>
                            <use fill="#ffbd9b" filter="url(#bbburst-blur-1)" opacity="0.6891592482245072"
                                 transform="matrix(0.7192485971798049,-0.6947528016891208,0.6947528016891208,0.7192485971798049,200.09702497503991,10.824222132722298)"
                                 width="24.01696828060861" xlink:href="#bbburst-shape-6"></use>
                            <use fill="#2c2891" filter="url(#bbburst-blur-4)" opacity="0.06917007541392017"
                                 transform="matrix(1.4859125012987524,0.9244263293979057,-0.9244263293979057,1.4859125012987524,1110.6216787030294,-416.7583293841506)"
                                 width="30.188856410028137" xlink:href="#bbburst-shape-4"></use>
                            <use fill="none" filter="url(#bbburst-blur-1)" opacity="0.7442708307775789" stroke="#ffa6d5"
                                 transform="matrix(0.9709135066915081,-0.23943049622802598,0.23943049622802598,0.9709135066915081,398.48398415074155,66.44945598785768)"
                                 width="16.414256106847827" xlink:href="#bbburst-shape-8">
                            </use>
                            <use fill="#ff5c58" filter="url(#bbburst-blur-1)" opacity="0.7169470847933221"
                                 transform="matrix(0.7338878192099325,0.6792706889122255,-0.6792706889122255,0.7338878192099325,602.7951378566083,-11.02629165460155)"
                                 width="18.495473361280155" xlink:href="#bbburst-shape-4"></use>
                            <use fill="#ffa6d5" filter="url(#bbburst-blur-1)" opacity="0.735162071800652"
                                 transform="matrix(0.8532308139238155,0.5215334871040433,-0.5215334871040433,0.8532308139238155,636.2521346192751,-60.453263555281865)"
                                 width="23.199458645077737" xlink:href="#bbburst-shape-6"></use>
                            <use fill="none" filter="url(#bbburst-blur-3)" opacity="0.3050173291592715" stroke="#2c2891"
                                 transform="matrix(1.243713709449363,0.1252046681546061,-0.1252046681546061,1.243713709449363,527.3253308109552,-396.7715008617972)"
                                 width="31.927193678380736" xlink:href="#bbburst-shape-10">
                            </use>
                            <use fill="#ffa6d5" opacity="0.8565374593855011"
                                 transform="matrix(0.7612440816706001,0.6484654563825931,-0.6484654563825931,0.7612440816706001,711.9013575126655,115.97386030286663)"
                                 width="17.21495340709956" xlink:href="#bbburst-shape-6"></use>
                            <use fill="#ffa6d5" filter="url(#bbburst-blur-2)" opacity="0.5491249080421088"
                                 transform="matrix(0.7828182237827475,0.6222504548119064,-0.6222504548119064,0.7828182237827475,816.7130201632856,-20.4368047485587)"
                                 width="21.300675210927587" xlink:href="#bbburst-shape-6"></use>
                            <use fill="#ff5c58" opacity="0.9567975308884051"
                                 transform="matrix(0.8254928265699611,0.5644126090738371,-0.5644126090738371,0.8254928265699611,632.1282479123586,50.63715406519566)"
                                 width="12.750636193702919" xlink:href="#bbburst-shape-4"></use>
                            <use fill="#2c2891" filter="url(#bbburst-blur-2)" opacity="0.5439326814941108"
                                 transform="matrix(0.9479827903420983,-0.31832158144745587,0.31832158144745587,0.9479827903420983,466.55726998494896,47.41907240067991)"
                                 width="21.147442236423064" xlink:href="#bbburst-shape-6"></use>
                            <use fill="#ffa6d5" opacity="0.7817985532892582"
                                 transform="matrix(0.9435282796747354,0.33129199424983735,-0.33129199424983735,0.9435282796747354,449.29475580032806,-41.51343903003203)"
                                 width="14.50546007293818" xlink:href="#bbburst-shape-4"></use>
                            <use fill="#ffa6d5" filter="url(#bbburst-blur-3)" opacity="0.26369648058379946"
                                 transform="matrix(1.1432906299798715,0.5053578290679074,-0.5053578290679074,1.1432906299798715,879.2081819168344,-215.90754508133122)"
                                 width="31.53109512636135" xlink:href="#bbburst-shape-4"></use>
                            <use fill="#2c2891" filter="url(#bbburst-blur-1)" opacity="0.6574734640415689"
                                 transform="matrix(0.8839657704185936,-0.467551619319474,0.467551619319474,0.8839657704185936,48.23166308074698,79.76010847443956)"
                                 width="29.22429181198515" xlink:href="#bbburst-shape-4"></use>
                            <use fill="none" filter="url(#bbburst-blur-3)" opacity="0.32071532830958127"
                                 stroke="#00a19d"
                                 transform="matrix(0.9036073289190667,-0.8637093232817101,0.8637093232817101,0.9036073289190667,308.54345686124566,196.95141044781445)"
                                 width="35.800005953016026" xlink:href="#bbburst-shape-10">
                            </use>
                            <use fill="none" filter="url(#bbburst-blur-3)" opacity="0.19414369178454016"
                                 stroke="#00a19d"
                                 transform="matrix(0.9354444327770889,0.8291222546683634,-0.8291222546683634,0.9354444327770889,419.55003643801797,-198.7240356953939)"
                                 width="35.147341067465746" xlink:href="#bbburst-shape-10">
                            </use>
                            <use fill="#ffa6d5" filter="url(#bbburst-blur-4)" opacity="0.0922658582232101"
                                 transform="matrix(1.4712039716747807,-0.9476596824432026,0.9476596824432026,1.4712039716747807,-202.07276038036494,185.17970459531637)"
                                 width="34.2680249182761" xlink:href="#bbburst-shape-4"></use>
                            <use fill="#00a19d" filter="url(#bbburst-blur-3)" opacity="0.16723031492059137"
                                 transform="matrix(1.2464004207971784,-0.09479446733125513,0.09479446733125513,1.2464004207971784,91.9807018823375,-379.66877636587924)"
                                 width="39.30061937181051" xlink:href="#bbburst-shape-1"></use>
                            <use fill="none" filter="url(#bbburst-blur-2)" opacity="0.5050053416666258" stroke="#ff5c58"
                                 transform="matrix(0.8902034346617496,0.45556321725576593,-0.45556321725576593,0.8902034346617496,345.0269175244564,33.09040937290604)"
                                 width="32.525247577707205" xlink:href="#bbburst-shape-10">
                            </use>
                            <use fill="#00a19d" filter="url(#bbburst-blur-2)" opacity="0.5266450285848325"
                                 transform="matrix(0.9269348917324046,0.3752222094826415,-0.3752222094826415,0.9269348917324046,394.4980794457795,-132.62206351085948)"
                                 width="25.325835692969747" xlink:href="#bbburst-shape-6"></use>
                            <use fill="#ffa6d5" filter="url(#bbburst-blur-3)" opacity="0.24740685437622134"
                                 transform="matrix(1.2308958610169427,-0.21770479859516084,0.21770479859516084,1.2308958610169427,485.32872120817655,-365.65858804536964)"
                                 width="35.8702206187409" xlink:href="#bbburst-shape-1"></use>
                            <use fill="none" filter="url(#bbburst-blur-1)" opacity="0.7266950733837458" stroke="#ffbd9b"
                                 transform="matrix(0.9351319122523568,-0.35429974130282743,0.35429974130282743,0.9351319122523568,235.8085304301906,-92.64820619517002)"
                                 width="21.1477870942505" xlink:href="#bbburst-shape-8">
                            </use>
                            <use fill="none" filter="url(#bbburst-blur-3)" opacity="0.23828492031492088"
                                 stroke="#ffbd9b"
                                 transform="matrix(1.2493757698868586,-0.03949918505006509,0.03949918505006509,1.2493757698868586,228.31174600877253,-414.263798924588)"
                                 width="33.86828563020525" xlink:href="#bbburst-shape-8">
                            </use>
                            <use fill="#ffa6d5" filter="url(#bbburst-blur-2)" opacity="0.46229668243450106"
                                 transform="matrix(0.8082334458384155,0.5888622054675958,-0.5888622054675958,0.8082334458384155,675.9661578983375,-167.13254546113185)"
                                 width="24.260175463994955" xlink:href="#bbburst-shape-6"></use>
                            <use fill="#ffa6d5" filter="url(#bbburst-blur-2)" opacity="0.3659016781653087"
                                 transform="matrix(0.9079905278799956,-0.41899069354844504,0.41899069354844504,0.9079905278799956,48.39470731069778,-186.92482101060082)"
                                 width="39.60864727470894" xlink:href="#bbburst-shape-4"></use>
                            <use fill="#ffbd9b" opacity="0.8677718748903157"
                                 transform="matrix(0.9642242479548268,-0.26508790929793247,0.26508790929793247,0.9642242479548268,314.95406633527693,68.96207951419393)"
                                 width="12.845945714783296" xlink:href="#bbburst-shape-1"></use>
                            <use fill="none" filter="url(#bbburst-blur-1)" opacity="0.5813242157731491" stroke="#ffa6d5"
                                 transform="matrix(0.7225997179824,0.691266697861076,-0.691266697861076,0.7225997179824,548.8926362605508,248.7239115700242)"
                                 width="25.013283693681192" xlink:href="#bbburst-shape-8">
                            </use>
                            <use fill="none" filter="url(#bbburst-blur-4)" opacity="-0.0787107941409122"
                                 stroke="#2c2891"
                                 transform="matrix(1.5869141817251493,0.7377014164549229,-0.7377014164549229,1.5869141817251493,1000.3441569132965,102.82561806958302)"
                                 width="37.369655570915" xlink:href="#bbburst-shape-10">
                            </use>
                            <use fill="none" opacity="0.8435151418189477" stroke="#ffa6d5"
                                 transform="matrix(0.8617576606551368,0.5073201497104033,-0.5073201497104033,0.8617576606551368,616.7349282081275,-16.32798776725315)"
                                 width="18.204280700796595" xlink:href="#bbburst-shape-10"></use>
                            <use fill="none" filter="url(#bbburst-blur-4)" opacity="-0.06786226156281772"
                                 stroke="#2c2891"
                                 transform="matrix(1.555381142228197,0.8020533039648361,-0.8020533039648361,1.555381142228197,344.6485292030916,101.12140506636297)"
                                 width="52.909754033282134" xlink:href="#bbburst-shape-10">
                            </use>
                            <use fill="#ffa6d5" filter="url(#bbburst-blur-2)" opacity="0.5535194468905071"
                                 transform="matrix(0.8536538733101535,0.5208407286134336,-0.5208407286134336,0.8536538733101535,734.8237015518342,-94.72843903889307)"
                                 width="25.210164615735618" xlink:href="#bbburst-shape-4"></use>
                            <use fill="#ff5c58" filter="url(#bbburst-blur-1)" opacity="0.6798741601967252"
                                 transform="matrix(0.7650407406891989,-0.6439818825756839,0.6439818825756839,0.7650407406891989,277.1024614188464,113.42129099078699)"
                                 width="23.21089456466773" xlink:href="#bbburst-shape-4"></use>
                            <use fill="#ffa6d5" filter="url(#bbburst-blur-2)" opacity="0.5143540555045474"
                                 transform="matrix(0.9246518483465198,0.38081354932481665,-0.38081354932481665,0.9246518483465198,754.8208019565492,-7.8641264495492464)"
                                 width="29.453174362143724" xlink:href="#bbburst-shape-4"></use>
                            <use fill="none" filter="url(#bbburst-blur-3)" opacity="0.18031701051195026"
                                 stroke="#2c2891"
                                 transform="matrix(1.2446114929006116,-0.11594063886192249,0.11594063886192249,1.2446114929006116,-33.00471385340908,-165.42743227666597)"
                                 width="39.28482030228723" xlink:href="#bbburst-shape-8">
                            </use>
                            <use fill="none" filter="url(#bbburst-blur-3)" opacity="0.1711035209732742" stroke="#2c2891"
                                 transform="matrix(1.2393915569652,0.16250713376149775,-0.16250713376149775,1.2393915569652,380.10268179042447,-463.86897768838196)"
                                 width="22.957759123804262" xlink:href="#bbburst-shape-10">
                            </use>
                            <use fill="#ffa6d5" opacity="0.8945395237101992"
                                 transform="matrix(0.8707097466284811,-0.49179725205227237,0.49179725205227237,0.8707097466284811,237.18502640823823,26.630134227198482)"
                                 width="10.634420968893473" xlink:href="#bbburst-shape-6"></use>
                            <use fill="none" filter="url(#bbburst-blur-1)" opacity="0.69906839424639" stroke="#ffa6d5"
                                 transform="matrix(0.937320653688959,-0.34846806477509906,0.34846806477509906,0.937320653688959,296.926600976792,156.02428054696634)"
                                 width="27.901568340528556" xlink:href="#bbburst-shape-8">
                            </use>
                            <use fill="none" filter="url(#bbburst-blur-1)" opacity="0.5939171074818335" stroke="#2c2891"
                                 transform="matrix(0.7651309467555046,-0.643874703895894,0.643874703895894,0.7651309467555046,203.39796261117954,-64.91376720739723)"
                                 width="28.46356298917959" xlink:href="#bbburst-shape-10">
                            </use>
                            <use fill="none" opacity="0.8434846417930874" stroke="#00a19d"
                                 transform="matrix(0.9999608545054298,-0.008848132953945107,0.008848132953945107,0.9999608545054298,457.936879986381,-16.74885511948395)"
                                 width="13.833516894509279" xlink:href="#bbburst-shape-10"></use>
                            <use fill="#ffa6d5" filter="url(#bbburst-blur-2)" opacity="0.37064590341358306"
                                 transform="matrix(0.8261383724810795,0.5634672923198942,-0.5634672923198942,0.8261383724810795,383.24971422012584,-112.12093093846366)"
                                 width="39.042004054710816" xlink:href="#bbburst-shape-6"></use>
                            <use fill="#ffa6d5" opacity="0.9318668027087007"
                                 transform="matrix(0.9902842584457509,-0.13905785657254058,0.13905785657254058,0.9902842584457509,366.57016720964907,18.080287843551446)"
                                 width="10.845372931377684" xlink:href="#bbburst-shape-4"></use>
                            <use fill="none" filter="url(#bbburst-blur-1)" opacity="0.6563346744277492" stroke="#00a19d"
                                 transform="matrix(0.9987433197183859,0.05011767470561549,-0.05011767470561549,0.9987433197183859,285.99329596246355,-96.73501546313622)"
                                 width="26.560629989897677" xlink:href="#bbburst-shape-10">
                            </use>
                            <use fill="#ffa6d5" opacity="0.8062571738749984"
                                 transform="matrix(0.7403798651803654,-0.6721887050787926,0.6721887050787926,0.7403798651803654,125.21549327426527,197.49949204728193)"
                                 width="19.228083856032107" xlink:href="#bbburst-shape-4"></use>
                            <use fill="#ff5c58" filter="url(#bbburst-blur-1)" opacity="0.6898394019102103"
                                 transform="matrix(0.9593825157087896,-0.2821084694798687,0.2821084694798687,0.9593825157087896,269.60249716861256,159.03137167330976)"
                                 width="23.737008492524538" xlink:href="#bbburst-shape-6"></use>
                            <use fill="#2c2891" filter="url(#bbburst-blur-1)" opacity="0.7176196163846472"
                                 transform="matrix(0.7344204170698702,0.6786948143244635,-0.6786948143244635,0.7344204170698702,604.8551750232273,211.75825169327402)"
                                 width="20.772909215259418" xlink:href="#bbburst-shape-1"></use>
                            <use fill="none" opacity="0.8237545673492574" stroke="#00a19d"
                                 transform="matrix(0.8461592203729731,-0.532930177206923,0.532930177206923,0.8461592203729731,157.95718632889378,-10.300218039849199)"
                                 width="16.476256174205822" xlink:href="#bbburst-shape-8"></use>
                            <use fill="#00a19d" filter="url(#bbburst-blur-2)" opacity="0.4072944788612678"
                                 transform="matrix(0.8673586207896838,-0.49768365749923654,0.49768365749923654,0.8673586207896838,57.023707619723666,295.9407845022058)"
                                 width="36.14229553272207" xlink:href="#bbburst-shape-4"></use>
                            <use fill="none" opacity="0.7899533466776378" stroke="#ffa6d5"
                                 transform="matrix(0.7494234714667896,-0.6620909759350795,0.6620909759350795,0.7494234714667896,186.1408350759649,181.16992381291857)"
                                 width="21.587245893708495" xlink:href="#bbburst-shape-10"></use>
                            <use fill="none" opacity="0.7984954393978203" stroke="#00a19d"
                                 transform="matrix(0.9994626353264713,-0.032778660531894044,0.032778660531894044,0.9994626353264713,443.5516221768584,-61.663433563390356)"
                                 width="18.67507546717053" xlink:href="#bbburst-shape-10"></use>
                            <use fill="none" filter="url(#bbburst-blur-4)" opacity="0.05845173186589514"
                                 stroke="#ff5c58"
                                 transform="matrix(1.3851717041909914,1.0694855538569112,-1.0694855538569112,1.3851717041909914,593.1893256234113,-533.0839111141656)"
                                 width="27.018259171683557" xlink:href="#bbburst-shape-8">
                            </use>
                            <use fill="#ffbd9b" opacity="0.9087164923894808"
                                 transform="matrix(0.8474439175036452,-0.5308849279138323,0.5308849279138323,0.8474439175036452,140.0540169907555,62.495908339104176)"
                                 width="15.598436966438527" xlink:href="#bbburst-shape-4"></use>
                            <use fill="none" opacity="0.9698618231186102" stroke="#2c2891"
                                 transform="matrix(0.7392290552133031,0.6734540844990454,-0.6734540844990454,0.7392290552133031,653.8163304145437,108.32668420953183)"
                                 width="14.164965963953524" xlink:href="#bbburst-shape-8"></use>
                            <use fill="none" opacity="0.8305660465380384" stroke="#ffa6d5"
                                 transform="matrix(0.7706754402639834,0.637227875860681,-0.637227875860681,0.7706754402639834,606.5233711385528,149.9841130432746)"
                                 width="18.238199423119443" xlink:href="#bbburst-shape-8"></use>
                            <use fill="none" filter="url(#bbburst-blur-4)" opacity="-0.09050395661000366"
                                 stroke="#ffa6d5"
                                 transform="matrix(1.5684329105859391,-0.7762204615899532,0.7762204615899532,1.5684329105859391,414.3658547015533,-547.4906020137084)"
                                 width="41.539052467513656" xlink:href="#bbburst-shape-8">
                            </use>
                            <use fill="none" opacity="0.9083274344896377" stroke="#00a19d"
                                 transform="matrix(0.9569628538359253,0.29021043464734597,-0.29021043464734597,0.9569628538359253,500.40264680386014,55.33965466233036)"
                                 width="16.349845582621192" xlink:href="#bbburst-shape-10"></use>
                            <use fill="#ff5c58" filter="url(#bbburst-blur-3)" opacity="0.25740832965588323"
                                 transform="matrix(0.9852006181519832,-0.7693372095466007,0.7693372095466007,0.9852006181519832,-221.73041989017224,173.86392314344806)"
                                 width="35.64569277451549" xlink:href="#bbburst-shape-4"></use>
                            <use fill="none" filter="url(#bbburst-blur-3)" opacity="0.14396842500670715"
                                 stroke="#2c2891"
                                 transform="matrix(1.2357356940674196,-0.18830107383579286,0.18830107383579286,1.2357356940674196,441.7037740256212,268.53430770515604)"
                                 width="34.05174598539992" xlink:href="#bbburst-shape-8">
                            </use>
                            <use fill="#ffa6d5" filter="url(#bbburst-blur-1)" opacity="0.66665291366817"
                                 transform="matrix(0.9666000297949311,-0.2562896455193583,0.2562896455193583,0.9666000297949311,390.4759451005265,123.29739446208686)"
                                 width="26.011017659943573" xlink:href="#bbburst-shape-4"></use>
                            <use fill="#00a19d" filter="url(#bbburst-blur-1)" opacity="0.6387157415381061"
                                 transform="matrix(0.9115005518203959,0.41129885002405936,-0.41129885002405936,0.9115005518203959,711.9447434170219,-9.754366283997399)"
                                 width="21.88722049534323" xlink:href="#bbburst-shape-4"></use>
                            <use fill="#2c2891" filter="url(#bbburst-blur-3)" opacity="0.17660321822992575"
                                 transform="matrix(0.9917241428224849,-0.7609094719761396,0.7609094719761396,0.9917241428224849,-285.8838932737252,95.45136351977766)"
                                 width="38.71217696186474" xlink:href="#bbburst-shape-1"></use>
                            <use fill="none" filter="url(#bbburst-blur-2)" opacity="0.4337541695089554" stroke="#ffa6d5"
                                 transform="matrix(0.8667441899578585,-0.4987529540506961,0.4987529540506961,0.8667441899578585,316.25519680460343,-157.65338337126423)"
                                 width="32.87086934061768" xlink:href="#bbburst-shape-10">
                            </use>
                            <use fill="#00a19d" filter="url(#bbburst-blur-3)" opacity="0.17522664985330927"
                                 transform="matrix(1.2499520695901698,0.01094640247987557,-0.01094640247987557,1.2499520695901698,744.1095507912003,-189.2388349535085)"
                                 width="32.89601614688991" xlink:href="#bbburst-shape-4"></use>
                            <use fill="#2c2891" filter="url(#bbburst-blur-3)" opacity="0.3266616966902913"
                                 transform="matrix(1.1595890680731706,-0.46674746191617983,0.46674746191617983,1.1595890680731706,177.68604165911415,-356.9275511145755)"
                                 width="41.520912394170445" xlink:href="#bbburst-shape-6"></use>
                            <use fill="none" filter="url(#bbburst-blur-4)" opacity="0.06674070438917634"
                                 stroke="#00a19d"
                                 transform="matrix(1.5832624156164523,0.745506621899736,-0.745506621899736,1.5832624156164523,305.51799244651846,-26.67270154182279)"
                                 width="47.59848009088071" xlink:href="#bbburst-shape-10">
                            </use>
                            <use fill="none" opacity="0.8489621477907312" stroke="#ffa6d5"
                                 transform="matrix(0.769523901642491,0.6386180116477439,-0.6386180116477439,0.769523901642491,590.7959639192078,120.36703934055606)"
                                 width="12.354823324618726" xlink:href="#bbburst-shape-8"></use>
                            <use fill="#00a19d" filter="url(#bbburst-blur-1)" opacity="0.7404527265010101"
                                 transform="matrix(0.9719737913453854,0.23508923611615465,-0.23508923611615465,0.9719737913453854,373.08986882189646,-20.849758861945702)"
                                 width="16.657637744352755" xlink:href="#bbburst-shape-6"></use>
                            <use fill="#00a19d" filter="url(#bbburst-blur-1)" opacity="0.5648413685746597"
                                 transform="matrix(0.9129891456833006,0.40798384755340134,-0.40798384755340134,0.9129891456833006,498.02296530723834,217.61290200904563)"
                                 width="30.08245795482894" xlink:href="#bbburst-shape-6"></use>
                            <use fill="#ffbd9b" filter="url(#bbburst-blur-1)" opacity="0.6232558986015112"
                                 transform="matrix(0.7432660398546442,-0.6689959596281538,0.6689959596281538,0.7432660398546442,279.0899382653495,179.65725717622468)"
                                 width="20.745148624702356" xlink:href="#bbburst-shape-6"></use>
                            <use fill="none" filter="url(#bbburst-blur-4)" opacity="0.07218670900225455"
                                 stroke="#00a19d"
                                 transform="matrix(1.2862386900234992,-1.1866296946759056,1.1866296946759056,1.2862386900234992,-462.9744833578062,-295.40315155268286)"
                                 width="35.746354001222855" xlink:href="#bbburst-shape-10">
                            </use>
                            <use fill="#ffa6d5" filter="url(#bbburst-blur-3)" opacity="0.18164077832966563"
                                 transform="matrix(1.2274866325431149,-0.23617063095983004,0.23617063095983004,1.2274866325431149,334.24040346919344,-452.34948676309904)"
                                 width="31.69924146312217" xlink:href="#bbburst-shape-6"></use>
                            <use fill="none" filter="url(#bbburst-blur-2)" opacity="0.4141589526058813" stroke="#ffbd9b"
                                 transform="matrix(0.97557237934913,-0.21967824801531255,0.21967824801531255,0.97557237934913,282.0142467267482,276.54462178381743)"
                                 width="32.21674853106318" xlink:href="#bbburst-shape-10">
                            </use>
                            <use fill="#00a19d" filter="url(#bbburst-blur-3)" opacity="0.11250386698903314"
                                 transform="matrix(1.247230224225941,0.08316710754444333,-0.08316710754444333,1.247230224225941,632.7593533533452,229.7244357998593)"
                                 width="40.13839269627302" xlink:href="#bbburst-shape-6"></use>
                            <use fill="#ffa6d5" filter="url(#bbburst-blur-3)" opacity="0.26131988150186636"
                                 transform="matrix(0.9629163577562899,-0.7970521237443389,0.7970521237443389,0.9629163577562899,122.61969526738875,357.0708362851349)"
                                 width="38.14297801018778" xlink:href="#bbburst-shape-1"></use>
                            <use fill="#ffa6d5" filter="url(#bbburst-blur-2)" opacity="0.3780422860311119"
                                 transform="matrix(0.7358481630516079,-0.6771465727104984,0.6771465727104984,0.7358481630516079,231.18469219077235,373.64756193320096)"
                                 width="40.25301122156451" xlink:href="#bbburst-shape-4"></use>
                            <use fill="#ff5c58" filter="url(#bbburst-blur-1)" opacity="0.7543657761560949"
                                 transform="matrix(0.7530564933292847,-0.6579558631508658,0.6579558631508658,0.7530564933292847,237.1665332479168,77.36524124916457)"
                                 width="17.97153101928128" xlink:href="#bbburst-shape-1"></use>
                            <use fill="#ffbd9b" filter="url(#bbburst-blur-2)" opacity="0.35496528824624163"
                                 transform="matrix(0.8859802309824817,-0.4637230103286103,0.4637230103286103,0.8859802309824817,485.159332015828,17.637886320495056)"
                                 width="38.93899852893418" xlink:href="#bbburst-shape-6"></use>
                            <use fill="#ffa6d5" filter="url(#bbburst-blur-3)" opacity="0.3302252654478107"
                                 transform="matrix(0.9848487168081757,-0.7697876363006161,0.7697876363006161,0.9848487168081757,374.23785335299885,68.03959723188433)"
                                 width="30.524344932514406" xlink:href="#bbburst-shape-6"></use>
                            <use fill="#ffa6d5" filter="url(#bbburst-blur-2)" opacity="0.40278727565196115"
                                 transform="matrix(0.8327493695007869,-0.5536501490978233,0.5536501490978233,0.8327493695007869,-36.51965243200101,253.78001168641953)"
                                 width="32.60627231860304" xlink:href="#bbburst-shape-1"></use>
                            <use fill="#00a19d" filter="url(#bbburst-blur-3)" opacity="0.15477060304291013"
                                 transform="matrix(0.8920306047294922,-0.8756605507991875,0.8756605507991875,0.8920306047294922,317.96276953919903,-193.28815883235225)"
                                 width="36.482632858792606" xlink:href="#bbburst-shape-4"></use>
                            <use fill="none" filter="url(#bbburst-blur-3)" opacity="0.24616458560912358"
                                 stroke="#ffa6d5"
                                 transform="matrix(1.156340676813612,0.4747380742538329,-0.4747380742538329,1.156340676813612,459.3513153493535,249.64893832570516)"
                                 width="41.974399813627386" xlink:href="#bbburst-shape-10">
                            </use>
                            <use fill="#00a19d" filter="url(#bbburst-blur-2)" opacity="0.549133718481291"
                                 transform="matrix(0.8035518346185494,-0.5952347848380195,0.5952347848380195,0.8035518346185494,257.4781735117211,259.75056943957725)"
                                 width="35.9305091370128" xlink:href="#bbburst-shape-6"></use>
                            <use fill="#2c2891" opacity="0.8647542431585912"
                                 transform="matrix(0.9690132101723151,-0.24700890370904574,0.24700890370904574,0.9690132101723151,244.9431267487355,-21.819595012711318)"
                                 width="14.939857097701156" xlink:href="#bbburst-shape-4"></use>
                            <use fill="#2c2891" filter="url(#bbburst-blur-4)" opacity="0.01851655686972109"
                                 transform="matrix(1.7306792388676078,0.25932483905834786,-0.25932483905834786,1.7306792388676078,730.4535692859321,-651.7807986153905)"
                                 width="43.38266316833932" xlink:href="#bbburst-shape-6"></use>
                            <use fill="#ffbd9b" filter="url(#bbburst-blur-3)" opacity="0.1503743315317576"
                                 transform="matrix(1.2486216482131127,0.05868542931230446,-0.05868542931230446,1.2486216482131127,761.7259014234368,33.567287052307734)"
                                 width="31.809735701830423" xlink:href="#bbburst-shape-4"></use>
                        </svg>
                    </div>
                </div>
                <div class="mt-4">
                    <h5 class="text-dark mb-0"><i class="ri-coins-fill fs-24 text-primary align-middle"></i> 19,343
                        Collected In This Month</h5>
                </div>
            </div>
        </div>
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">Property Photos</h4>
            </div>
            <div class="card-body">
                <div class="carousel slide" data-bs-ride="carousel" id="carouselExampleCaptions">
                    <div class="carousel-inner">
                        <div class="carousel-item active">
                            <img alt="img-6" class="d-block w-100 rounded"
                                 src="{{ asset('assets/admin') }}/images/properties/p-1.jpg"/>
                            <div class="carousel-caption d-none d-md-block bg-light rounded p-2 text-start">
                                <div class="d-flex align-items-center gap-2">
                                    <div class="avatar bg-primary rounded">
                                        <iconify-icon class="fs-24 text-white avatar-title"
                                                      icon="solar:home-bold-duotone"></iconify-icon>
                                    </div>
                                    <div>
                                        <a class="text-dark fw-medium fs-16" href="#!">Dvilla
                                            Residences Batu</a>
                                        <p class="text-muted mb-0">4604 , Philli Lane
                                            Kiowa</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="carousel-item">
                            <img alt="img-7" class="d-block w-100 rounded"
                                 src="{{ asset('assets/admin') }}/images/properties/p-2.jpg"/>
                            <div class="carousel-caption d-none d-md-block bg-light rounded p-2 text-start">
                                <div class="d-flex align-items-center gap-2">
                                    <div class="avatar bg-primary rounded">
                                        <iconify-icon class="fs-24 text-white avatar-title"
                                                      icon="solar:home-bold-duotone"></iconify-icon>
                                    </div>
                                    <div>
                                        <a class="text-dark fw-medium fs-16" href="#!">PIK
                                            Villa House</a>
                                        <p class="text-muted mb-0">27, Boulevard
                                            Cockeysville</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="carousel-item">
                            <img alt="img-5" class="d-block w-100 rounded"
                                 src="{{ asset('assets/admin') }}/images/properties/p-3.jpg"/>
                            <div class="carousel-caption d-none d-md-block bg-light rounded p-2 text-start">
                                <div class="d-flex align-items-center gap-2">
                                    <div class="avatar bg-primary rounded">
                                        <iconify-icon class="fs-24 text-white avatar-title"
                                                      icon="solar:home-bold-duotone"></iconify-icon>
                                    </div>
                                    <div>
                                        <a class="text-dark fw-medium fs-16" href="#!">Tungis
                                            Luxury</a>
                                        <p class="text-muted mb-0">900 , Creside WI 54913
                                        </p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="carousel-item">
                            <img alt="img-5" class="d-block w-100 rounded"
                                 src="{{ asset('assets/admin') }}/images/properties/p-4.jpg"/>
                            <div class="carousel-caption d-none d-md-block bg-light rounded p-2 text-start">
                                <div class="d-flex align-items-center gap-2">
                                    <div class="avatar bg-primary rounded">
                                        <iconify-icon class="fs-24 text-white avatar-title"
                                                      icon="solar:buildings-3-bold-duotone"></iconify-icon>
                                    </div>
                                    <div>
                                        <a class="text-dark fw-medium fs-16" href="#!">Luxury
                                            Apartment</a>
                                        <p class="text-muted mb-0">223 , Creside Santa
                                            Maria</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="carousel-item">
                            <img alt="img-5" class="d-block w-100 rounded"
                                 src="{{ asset('assets/admin') }}/images/properties/p-5.jpg"/>
                            <div class="carousel-caption d-none d-md-block bg-light rounded p-2 text-start">
                                <div class="d-flex align-items-center gap-2">
                                    <div class="avatar bg-primary rounded">
                                        <iconify-icon class="fs-24 text-white avatar-title"
                                                      icon="solar:home-bold-duotone"></iconify-icon>
                                    </div>
                                    <div>
                                        <a class="text-dark fw-medium fs-16" href="#!">Weekend
                                            Villa MBH</a>
                                        <p class="text-muted mb-0">980, Jim Rosa Lane
                                            Dublin</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <button class="carousel-control-prev" data-bs-slide="prev" data-bs-target="#carouselExampleCaptions"
                            type="button">
                        <span aria-hidden="true" class="carousel-control-prev-icon"></span>
                        <span class="visually-hidden">Previous</span>
                    </button>
                    <button class="carousel-control-next" data-bs-slide="next" data-bs-target="#carouselExampleCaptions"
                            type="button">
                        <span aria-hidden="true" class="carousel-control-next-icon"></span>
                        <span class="visually-hidden">Next</span>
                    </button>
                </div>
            </div>
        </div>
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">Location</h4>
            </div>
            <div class="card-body">
                <div class="my-4" id="agent-location" style="height: 228px"></div>
                <div class="d-flex align-items-center bg-light justify-content-between p-3 rounded">
                    <div>
                        <h5 class="fw-medium mb-1 text-dark">Walker Art Center</h5>
                        <p class="mb-2">Lincoln Drive Harrisburg, PA 17101 U.S.A</p>
                        <div class="d-flex gap-2 align-items-center">
                            <ul class="d-flex text-warning m-0 fs-18 list-unstyled">
                                <li>
                                    <i class="ri-star-fill"></i>
                                </li>
                                <li>
                                    <i class="ri-star-fill"></i>
                                </li>
                                <li>
                                    <i class="ri-star-fill"></i>
                                </li>
                                <li>
                                    <i class="ri-star-fill"></i>
                                </li>
                                <li>
                                    <i class="ri-star-half-line"></i>
                                </li>
                            </ul>
                            <p class="mb-0 fw-medium fs-15 text-dark">4.5 <span
                                    class="text-muted fs-13">(5809 Review)</span></p>
                        </div>
                    </div>
                    <div>
                        <div class="avatar bg-dark rounded">
                            <iconify-icon class="fs-30 text-light avatar-title"
                                          icon="solar:point-on-map-perspective-bold"></iconify-icon>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
@endsection

@section('extra_javascript')
<script src="{{ asset('assets/admin') }}/vendor/jsvectormap/jsvectormap.min.js"></script>
<script src="{{ asset('assets/admin') }}/vendor/jsvectormap/maps/world-merc.js"></script>
<script src="{{ asset('assets/admin') }}/vendor/jsvectormap/maps/world.js"></script>
<script src="{{ asset('assets/admin') }}/js/pages/agent-detail.js"></script>
@endsection
